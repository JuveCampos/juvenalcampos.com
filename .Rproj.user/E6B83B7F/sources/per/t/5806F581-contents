...
El post de arriba fué el post de prueba con el que estuve probando la página durante la primera semana. Hacer un blog es una idea que había estado rondandome la cabeza durante ya un año, desde que me comprometí públicamente en un Tweet a escribirlo como uno de mis propósitos de R de año nuevo (al cual incluso el mismo Hadley Wickham le dió RT). 
...
Hoy, en enero del 2020 y tras las vacaciones de Diciembre (y una gripa espantosa que me tuvo en cama pensando muchas cosas) me decidí a armar un blog con la librería `blogdown` de RStudio y de una vez, empezar a publicar cositas, tanto de R como de otros temas de mi interés (que, al final de cuentas, si me interesan a mi muy probablemente le interesen a alguien mas).

## Elaboración un blog con Blogdown, platicadito.

_(Próximamente el tutorial)._

`Blogdown` es una librería que, mediante el uso de funciones de R y el Entorno RStudio, nos facilita la elaboración de blogs al crear publicaciones de manera cómoda y sin tener que instalar otro programa adicional. 

Si bien aprendí lo mínimo indispensable para poder montar el sitio y poderle hacer modificaciones (para lo cual también me sirvió mi entrenamiento en desarrollo web derivado de aprender de la librería `shiny`), próximamente espero sacar el documento o tutorial en el cual explique de manera más técnica el funcionamiento interno. 
